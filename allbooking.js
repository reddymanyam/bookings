...
externalNewbookingBtn.addEventListener("click", function () {
    my_modal_3_external_client_booking.showModal();
    //Add dynamic src to iframe
    let iframe = document.getElementById("iframe");
    let url = `${window.location.href}/external_client_booking`
    iframe.src = url;
});
...
let fromDateInput = document.getElementById("booking_from_date_filter");
fromDateInput.value = currentDate;

let toDateInput = document.getElementById("booking_to_date_filter");
let fromDate = fromDateInput.value;
let toDate = toDateInput.value;

// If "toDate" is less than "fromDate" (currentDate), update it to match "fromDate"
...
function updateDetails() {
    ...
    let bookingDate = document.getElementById('booking_from_date_filter').value;
...
document.getElementById('booking_from_date_filter').addEventListener('change', updateDetails);
...
function main() {
    ...
    const getFilterValues = () => {
        return {
            ...
            bookingDate: document.getElementById('booking_from_date_filter').value
        };
    };

    fetchData(currentPagePending, false, storedLocation, null, null, formattedDate);
    fetchTotalPages(storedLocation, null, null, formattedDate); // Fetch total pages first
    ...
    document.querySelector(".prev-page-pending").addEventListener("click", () => {
        if (currentPagePending > 1) {
            currentPagePending--;
            const { location, roomType, room, bookingDate } = getFilterValues();

            fetchData(currentPagePending, false, location, roomType, room, bookingDate);
            fetchTotalPages(location, roomType, room, bookingDate); // Fetch total pages
...
async function fetchTotalPages(location, roomType, room, dates) {
...
async function fetchData(pagePending, checkData = false, location, roomType, room, dates) {
...
function constructTable(data, slNo, tableName) {
    ...
    tableRow.innerHTML = `
        ...
        <td>${data.card_status}</td>
        <td>${data.money_collected}</td>
    `;
...
document.getElementById('customer').addEventListener("change", function(e){
    fetchLeadsForCustomer(e.target.value);
});
...
function fetchCustomerEmails(customerName) {
    frappe.call({
        method: "frappe.client.get_list", // Use get_list to retrieve multiple records
        args: {
            doctype: 'User',
            fields: ['email'], // Fetch the email field
            filters: [
        ...
        callback: function (response) {
            const emailData = response.message; // Now emailData will be an array of objects
            const emailSelectElement = document.getElementById('email');
            
            // Clear previous email options
            emailSelectElement.innerHTML = '<option value="">Select Email</option>';

            if (emailData && emailData.length > 0) {
                // Loop through emailData, which is an array of user objects
                emailData.forEach(item => {
                    const email = item.email; // Extract the email from the response
                    const option = document.createElement('option');
                    option.value = email;
                ...
                emailSelectElement.innerHTML = '<option value="">No emails found</option>';
                alert('No emails found for this customer.');
            }
        }
    });
} 
...
// Event listeners for customer, lead, and email search fields
document.getElementById('all_customer').addEventListener('input', function () {
    const query = this.value;
    customer_name = this.value;
    fetchFormData('Customer', 'customer_name', 'allCustomerSuggestions', [['customer_name', 'like', `%${query}%`]]);
});
...
function getComplimentary(lead_id, user_room_type, date, price_per_hour, bookedHours) {
    ...
    frappe.call({
        ...
        callback: function (response) {
            if (response && response.message) {
                const complimentary_hours_available = response.message.complimentary_avaliable === "No complimentary hours" ? 0 : response.message.complimentary_avaliable;
                let billableHours = 0;
                let price = 0;
                ...
                if (complimentary_hours_available >= bookedHours) {
                    ...
                    total_price.innerHTML = price + gst;
                    total_booking_price = price + gst;
                    bookingData.price = price + gst;
                }
            }
        },
        error: function (err) {
