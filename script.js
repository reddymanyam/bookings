...
//Billing Details
const complimentary_available = document.getElementById("complementary_available");
...
function appendDetails(data) {
    ...
    // Get the current date at midnight (00:00:00) to ignore the time part in the comparison
    const currentDate = new Date();
    const currentDateOnly = new Date(currentDate.toISOString().split('T')[0]); // Set time to 00:00:00
...
function constructTable(data, slNo, tableName) {
    ...
    tableRow.innerHTML = `
        ...
        <td class="font-bold ${data.client_type != 'Deposit' ? 'text-blue-700' : ''}">${data.client_type}</td>
        <td class="text-green-800 font-medium">${startTime}</td>
        <td class="text-red-800 font-medium">${endTime}</td>
        <td class="font-bold ${data.client_type != 'Deposit' ? 'text-blue-700' : ''}">${data.price}</td>
...
document.getElementById('lead_id').addEventListener('change', function () {
    ...
    if (lead.length === 1) {
        ...
        if (locations[locationCode]) {
            document.getElementById('billing_location').value = locations[locationCode];
            Billing_location_of_client = locations[locationCode];
        } else {
            document.getElementById('billing_location').value = "Location not found.";
        }
    } else {
...
function getComplimentary(lead_id, user_room_type, date, price_per_hour) {
    ...
    frappe.call({
        ...
        callback: function (response) {
            if (response && response.message) {
                console.log("Complimentary Details = ", response.message);
                const complimentary_hours_available = response.message === "No complliementary hours" ? 0 : response.message;
                ...
                if (complimentary_hours_available >= bookedHours) {
                    ...
                    console.log("price_per_hour = ", price_per_hour);
...
function showDetails_of_invoice(formData) {
    ...
    getComplimentary(formData.lead_id, formData.room_type, formData.booking_date, formData.rate);
    ...
    frappe.call({
        ...
        args: {
            doc: {
                ...
                money_collected: bookingData.money_collected,
                money_collected_date: bookingData.money_collected_date,
                card_status: bookingData.card_status,
                block_temp: 0
            }
        },
        callback: function (response) {
